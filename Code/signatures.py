...
def MMS_row(n, num_steps, dim):
    return np.empty(n * num_steps * dim * (dim - 1) // 2, dtype="float32").reshape(
        (n, num_steps * dim * (dim - 1) // 2)
    )
...
def quantile(M, dim):
    return np.vstack(
        [
            np.quantile(
                M, np.arange(1 / q, 1 - 1e-8, 1 / q), axis=1, interpolation="midpoint"
            )
            for q in np.arange(2, dim + 1)
        ]
    ).T
...
def mixing_matrix_signature(Evec, Eval, t_min, t_max, dim, threshold):
    ...
    t_interval = np.logspace(
        np.log10(t_min), np.log10(t_max), num=num_steps, endpoint=True
    )
    ...
    pairwise_subs = (unique_eval[:, np.newaxis] - unique_eval[np.newaxis, :])[
        uti_r, uti_c
    ].astype("complex64")
    ...
    for j, t in enumerate(t_interval):
		...
		"""
        MMSrow[
            :, np.arange(j * dim * (dim - 1) // 2, (j + 1) * dim * (dim - 1) // 2)
        ] = quantile(M, dim)
...
def heat_kernel_signature(Evec, Eval, dim):
    ...
    t_interval = np.exp(
        np.linspace(np.log(tmin), np.log(tmax), num=num_steps, endpoint=True)
    )
    ...
    for i, t in enumerate(t_interval):
        ...
        Q = np.quantile(
            H, np.arange(1 / dim, 1 - 1e-8, 1 / dim), axis=1, interpolation="midpoint"
        )
        H = np.fliplr(Q.T)
        # (np.sort(Q.T, axis=1), axis=1)
        HKSrow[:, np.arange(i * (dim - 1), (i + 1) * (dim - 1))] = H[
            :, np.arange(dim - 1)
        ]
    ...
    for i in range(N):
        WKS[:, i] = np.sum(
            np.multiply(
                np.power(PHI, 2),
                np.tile(
                    np.exp(-np.power((e[i] - log_E), 2) / (2 * sigma**2)), (n, 1)
                ),
            ),
            axis=1,
        ).ravel()
