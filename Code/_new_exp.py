...
t_min = 1e-1
t_max = np.linspace(1, 20, num=10, endpoint=True) # go to 100
...
=> outcomeHW[experiment][quantile][HKS, errHKS, WKS, errWKS]
=> outcomeMMS[experiment][quantile][t_max][threshold][MMSdiag, errMMSdiag, MMSrow, errMMSrow]
"""
...
rounds_per_exp = n * (n - 1) // 2
tot = rounds_per_exp  * len(quantiles) * len(thresholds) * len(t_max)
i = 0

avg_discarded = 0.0
pair = 0
resultHW = np.empty((len(quantiles), 2))
resultMMS = np.empty((len(quantiles), len(t_max), len(thresholds), 2))
