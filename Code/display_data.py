...
for dataset in real:
    ...
    data = np.hstack(
        [dmin[:, :4], d5[:, :2], d10[:, :2], dmin[:, 4:], d5[:, 4:], d10[:, 4:]]
    )
    ...
    print("-" * len(dataset))
    print(
        "       HKSmin WKS    HKSd5  HKSd10 MMRmin MMDmin MMRd5  MMDd5  MMRd10 MMDd10"
    )
...
for dataset in synthetic:
    ...
    m = np.hstack(
        [hw[0, 0], hw[0, 2]]
        + [hw[q, 0] for q in range(1, qs)]
        + [mms[q, 0, 0, ::2] for q in range(qs)]
    )
    sd = np.hstack(
        [hw[0, 1], hw[0, 3]]
        + [hw[q, 1] for q in range(1, qs)]
        + [mms[q, 0, 0, 1::2] for q in range(qs)]
    )
    print(dataset)
    print("-" * len(dataset))
    print(
